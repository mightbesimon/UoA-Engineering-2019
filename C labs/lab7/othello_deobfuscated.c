...
//#define U Z printf
int functionE(int*v,int z,int f,int o);		// what the heck is this?
/*#define Z printf("123\
45678\n");
for (x=V+11 ; x<V+89 ; x++) {
	putchar(".XO"[*x]);
...
int functionS(int d,int*v,int f,int _,int a,int b)
{
	...
	for (z=11 ; z<100 ; z++) {
		if (functionE(v,z,f,100)){
			c++;
			w = -functionS(d+1,n,q,0,-b,-j);	// what the heck? recursive call?
			if (w > j) {
				g = bz=z;
				j = w;
				if ((w>=b) || (w>=8003)) {
	...
	if (!c) {
		...
		j = -functionS(d+1,n,q,1,-b,-j);		// ????? recursive again?
	}
	bz = g;
...
int main(void)
{
	...
	while (lv > 0) {
		...
		printf("Wait\n");
		printf("Value:%d\n" , functionS(0,V,1,0,-9000,9000));
		printf("move: %d\n" , (lv -= functionE(V,bz,1,0) , bz));
	}

	return 0;		// added this here
}




int functionE(int*v,int z,int f,int o) 
{
